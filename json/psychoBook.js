export default {
 "Books": [
  {
   "id": 1,
   "name": "溝通分析心理學經典, 改變你人生結局的必讀雙巨作: 人間遊戲+人生腳本 (2冊合售)",
   "author": "艾瑞克．伯恩",
   "publishingHouse": "小樹文化有限公司",
   "publishingDate": "2022/03/30",
   "price": 250,
   'img': './psybook/1'
  },
  {
   "id": 2,
   "name": "魔鬼學: 從無意識到憂鬱、自戀、死本能",
   "author": "佛洛伊德",
   "publishingHouse": "心靈工坊文化事業股份有限公司",
   "publishingDate": "2022/11/02",
   "price": 184,
   'img': './psybook/2'

  },
  {
   "id": 3,
   "name": "一口氣讀完26本心理學經典 ",
   "author": "宋學軍",
   "publishingHouse": "海鴿文化出版圖書有限公司",
   "publishingDate": "2022/10/26",
   "price": 118,
   'img': './psybook/3'
  },
  {
   "id": 4,
   "name": "我僅僅只是一個胖子",
   "author": "蔡培元",
   "publishingHouse": "大塊文化出版股份有限公司",
   "publishingDate": "2022/10/29",
   "price": 77,
   'img': './psybook/4'
  },
  {
   "id": 5,
   "name": "宛轉人生向前看: 從洗頭妹到坐擁百萬粉絲的奇幻人生",
   "author": "宛宛兒",
   "publishingHouse": "大塊文化出版股份有限公司",
   "publishingDate": "2022/10/29",
   "price": 88,
   'img': './psybook/5'
  },
  {
   "id": 6,
   "name": "解讀夢境的心理學: 從做夢到懂夢, 探索不可思議的夢境世界, 成為自己的最佳解夢人!",
   "author": "松田英子",
   "publishingHouse": "晨星出版有限公司",
   "publishingDate": "2022/08/12",
   "price": 80,
   'img': './psybook/6'
  },
  {
   "id": 7,
   "name": "心理學家的工具箱: 疏導負面情緒的10大溝通法則",
   "author": "香港心理學會輔導心理學部",
   "publishingHouse": "萬里機構．萬里書店",
   "publishingDate": "2021/10/22",
   "price": 300,
   'img': './psybook/7'
  },
  {
   "id": 8,
   "name": "改變潛意識, 成就美好現實: 用量子力學實現願望的11個法則",
   "author": "高橋宏和",
   "publishingHouse": "世茂出版有限公司",
   "publishingDate": "2022/11/02",
   "price": 300,
   'img': './psybook/8'

  },
  {
   "id": 9,
   "name": "別讓自責成為一種習慣: 放過自己的100個正向練習",
   "author": "根本裕幸",
   "publishingHouse": "聯經出版事業股份有限公司",
   "publishingDate": "2022/11/03",
   "price": 250,
   'img': './psybook/9'
  },
  {
   "id": 10,
   "name": "不完美的我, 也是最好的自己: 哈佛精神科醫師執業45年的良心告白, 接受人生就是不公平, 練習做自己情緒的主人 (第3版)",
   "author": "麥可．班奈特/ 莎拉．班奈特",
   "publishingHouse": "奇光出版",
   "publishingDate": "2022/11/02",
   "price": 320,
   'img': './psybook/10'
  },
  {
   "id": 11,
   "name": "超深層心理測驗: 50個揭開心靈實相的潛意識測驗",
   "author": "中嶋真澄",
   "publishingHouse": "楓書坊文化出版社",
   "publishingDate": "2020/04/01",
   "price": 270,
   'img': './psybook/11'
  },
  {
   "id": 12,
   "name": "模仿欲望: 從人性深處理解商業、政治、經濟、社會現象, 還有你自己",
   "author": "柏柳康",
   "publishingHouse": "遠見天下文化出版股份有限公司",
   "publishingDate": "2022/10/31",
   "price": 330,
   'img': './psybook/12'
  }
 ]
}